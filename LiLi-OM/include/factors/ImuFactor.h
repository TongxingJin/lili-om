...
public:
    ...
    ImuFactor() = delete;
    ImuFactor(Preintegration *pre_integration) : pre_integration_{pre_integration} {
    ...
    virtual bool Evaluate(double const *const *parameters, double *residuals, double **jacobians) const {
        ...
        if(jacobians) {
            ...
            if (jacobians[1]) {
                ...
                jacobian_pose_qi.block<3, 4>(O_R, 0) = -2 * (Qleft(Qj.inverse()) * Qright(corrected_delta_q)).bottomRightCorner<3, 4>();
